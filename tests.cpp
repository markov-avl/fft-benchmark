...
size_t get_testing_sequence_size(const IFourierTransformAlgorithm *algorithm) {
    ...
    if (algorithm->supported_sequences() == ANY) {
        ...
        if (algorithm->supported_sequences() & RADIX_3) {
            n *= 2187;
        }
        if (algorithm->supported_sequences() & RADIX_4) {
            n *= 65536;
        }
        if (algorithm->supported_sequences() & RADIX_5) {
            n *= 3125;
        }
        if (algorithm->supported_sequences() & RADIX_8) {
            n *= 262144;
        }
    }
